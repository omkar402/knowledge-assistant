  ...
  <div class="min-h-screen flex bg-dark-50 dark:bg-dark-900">
    ...
    <div class="hidden lg:flex lg:w-5/12 bg-primary-600 dark:bg-primary-700 flex-col items-center justify-center p-12 relative overflow-hidden">
      ...
      <div class="relative text-center">
        ...
        <p class="text-primary-100 text-base leading-relaxed max-w-xs">
          Your AI-powered research workspace. Upload sources, ask questions, and get answers grounded in your documents.
        </p>
      </div>
    </div>
    ...
    <div class="flex-1 flex items-center justify-center p-6">
      <div class="w-full max-w-md">
        ...
        <div class="bg-white dark:bg-dark-800 rounded-2xl shadow-sm border border-dark-200 dark:border-dark-700 p-8">
          <h2 class="text-2xl font-bold text-dark-900 dark:text-white mb-1">Welcome back</h2>
          <p class="text-dark-500 text-sm mb-8">Sign in to your workspace</p>
          ...
          <form @submit.prevent="handleSubmit" novalidate class="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                :class="['input', fieldError('email') ? 'input-error' : '']"
                @blur="validateField('email')"
                @input="clearError('email')"
              />
              <p v-if="fieldError('email')" class="mt-1.5 text-xs text-red-600 dark:text-red-400 flex items-center gap-1">
            ...
            <div>
              <div class="flex items-center justify-between mb-1.5">
                <label for="password" class="block text-sm font-medium text-dark-700 dark:text-dark-300">
                  Password
                </label>
                <a href="#" class="text-xs text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300 transition-colors">
                  Forgot password?
                </a>
              </div>
              <div class="relative">
                <input
                  ...
                  placeholder="••••••••"
                  :class="['input pr-10', fieldError('password') ? 'input-error' : '']"
              ...
              <p v-if="fieldError('password')" class="mt-1.5 text-xs text-red-600 dark:text-red-400 flex items-center gap-1">
                <ExclamationCircleIcon class="w-3.5 h-3.5 flex-shrink-0" />
                {{ fieldError('password') }}
              </p>
            </div>
            ...
            <button
              type="submit"
              :disabled="loading"
              class="btn-primary w-full justify-center py-2.5 text-sm mt-2"
            >
          ...
          <p class="mt-6 text-center text-sm text-dark-500">
            ...
            <router-link to="/register" class="text-primary-600 hover:text-primary-500 dark:text-primary-400 font-medium transition-colors">
              Create one
            </router-link>
          </p>
        </div>
...
function fieldError(field) { return errors[field] }
